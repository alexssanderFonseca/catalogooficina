# 📦 Serviço de Catálogo de Peças e Insumos

Microserviço responsável pelo gerenciamento de peças e insumos da oficina, cobrindo o cadastro, listagem, busca, atualização, exclusão e controle de estoque de itens.

---

## 📋 Descrição

O **Serviço de Catálogo de Peças e Insumos** é um microserviço construído em **Java 21** com **Spring Boot** e **PostgreSQL**. A sua arquitetura é baseada no padrão **Ports and Adapters (Arquitetura Hexagonal)**, que isola a lógica de negócio de detalhes de infraestrutura.

O sistema foi projetado para simular processos típicos de uma oficina, como:

-   Gerenciamento de peças e insumos (cadastro, busca, atualização, exclusão)
-   Controle de estoque (dar baixa)
...
- [**Collection Insomnia - Catalogo**](docs/collections/catalogo-insomnia-collection.yaml) <!-- Placeholder: Update with actual path if exists -->
...
1.  Navegue até o diretório `catalogo/`.
...
2.  Construa a imagem Docker do microserviço:
    ```bash
    docker build -t alexmarquesfa/catalogooficina:latest .
    ```
    (Substitua `alexmarquesfa/catalogooficina` pelo nome da sua imagem no Docker Hub, se aplicável).
3.  Aplique os manifestos Kubernetes usando Kustomize a partir da raiz do projeto:
    ```bash
    kubectl apply -k catalogo/k8s/base/
    ```
    Ou, para um overlay específico (ex: ambiente de desenvolvimento):
    ```bash
    kubectl apply -k catalogo/k8s/overlays/dev/ # ou producao, homolog, etc.
    ```

Após alguns instantes, todos os recursos (Pods, Services, Deployments, etc.) para o serviço de `catalogo` estarão sendo criados.
...
```bash
minikube service catalogo-app-service -n oficina-ns
```

#### 7. (Opcional) Monitoramento e Limpeza

- **Verificar status dos Pods:** `kubectl get pods -n oficina-ns -l app=catalogo-app`
...
O projeto utiliza GitHub Actions para automação de integração e deploy contínuo. Os workflows específicos para este microserviço estão definidos em `.github/workflows` dentro do diretório `catalogo/`.

### CI - Integração Contínua (`catalogo/.github/workflows/ci-pipeline.yml`)

Este pipeline é acionado a cada `push` em uma branch com o padrão `feature/*` e quando há alterações no diretório `catalogo/`. Seu objetivo é garantir a qualidade e a integridade do código antes que ele seja mesclado à `main`.
...
### CD - Deploy Contínuo (`catalogo/.github/workflows/cd-pipeline.yml`)

Este pipeline é acionado automaticamente após um `merge` na branch `main` e quando há alterações no diretório `catalogo/`. Seu objetivo é colocar a nova versão da aplicação em produção no ambiente da AWS.
